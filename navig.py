...
def ajouterItemAuMenu(items):
    ...
    xbmc.executebuiltin('Container.SetSortDirection(1)')
    
    for item in items:
        if item['isDir'] == True:
            ajouterRepertoire(item)
            xbmcplugin.addSortMethod(__handle__, xbmcplugin.SORT_METHOD_LABEL_IGNORE_THE)
            #xbmc.executebuiltin('Container.SetViewMode('+str(xbmcplugin.SORT_METHOD_LABEL_IGNORE_THE)+')')
            #xbmc.executebuiltin('Container.SetSortDirection(1)')
